...
async function analizarConGemini(dataUrl: string) {
  ...
  const prompt = `Eres un barbero moderno especializado en tendencias de hombre 2026. Analiza esta foto y recomienda 3 cortes ACTUALES Y DE MODA (Ej: Burst Fade, Mid Fade con textura, Crop francés, Edgar, Fringe lateral, Buzz Cut, Two-block). Evita cortes anticuados (NO pompadour, NO mullet, NO estilos años 80) y NO exageres el volumen. Los 3 cortes deben ser **diferentes entre sí**: uno corto/rapado, uno con flequillo textura corto-medio, y uno medio con un poco de volumen NATURAL. Responde SOLO con JSON válido, sin markdown.
{"forma":"Nombre forma cara (Ovalada/Redonda/Cuadrada/Rectangular/Corazón/Diamante/Triangular)","emoji":"emoji representativo","descripcion":"1-2 frases motivadoras sobre la forma detectada","cortes":[{"nombre":"nombre del corte","promptIngles":"Change ONLY the hairstyle to [descripción del corte en inglés, describe length around 2-5cm max on top, fade type, texture]. Modern trendy 2026 men's haircut, realistic natural hair length, NOT exaggerated volume, NOT puffy, hair stays close to the head. Keep the EXACT same face, same skin, same eyes, same expression, same lighting, same background. DO NOT add beard, DO NOT add facial hair, DO NOT change skin tone, DO NOT change age.","descripcion":"1-2 frases por qué le favorece"},{"nombre":"nombre corte 2","promptIngles":"Change ONLY the hairstyle to ...","descripcion":"descripción"},{"nombre":"nombre corte 3","promptIngles":"Change ONLY the hairstyle to ...","descripcion":"descripción"}]}`
