...
async function analizarConGemini(dataUrl: string) {
  ...
  const prompt = `Eres un barbero moderno especializado en tendencias de hombre 2026. Analiza esta foto y recomienda 3 cortes ACTUALES Y DE MODA (Ej: Burst Fade, Mid Fade con textura, Crop francés, Mullet moderno, Two-block, Buzz Cut, Edgar/Takuache, Fringe lateral, Slick Back). Evita cortes anticuados o muy exagerados (no pompadour gigante, no estilos años 80). Los 3 cortes deben ser **diferentes entre sí**: uno corto/rapado, uno con flequillo o textura medio, y uno con más volumen pero realista. Responde SOLO con JSON válido, sin markdown.
{"forma":"Nombre forma cara (Ovalada/Redonda/Cuadrada/Rectangular/Corazón/Diamante/Triangular)","emoji":"emoji representativo","descripcion":"1-2 frases motivadoras sobre la forma detectada","cortes":[{"nombre":"nombre del corte de moda","promptIngles":"Change only the hairstyle to a [descripción REALISTA y NATURAL del corte de moda actual en inglés, describe length, texture, sides, top]. Modern trendy 2026 men's haircut, realistic and natural, not exaggerated. Keep the exact same face, same skin, same eyes, same expression, same lighting, same background.","descripcion":"1-2 frases por qué le favorece"},{"nombre":"nombre corte 2","promptIngles":"Change only the hairstyle to ...","descripcion":"descripción"},{"nombre":"nombre corte 3","promptIngles":"Change only the hairstyle to ...","descripcion":"descripción"}]}`
