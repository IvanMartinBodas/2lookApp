  ...
  <ion-page>
    <ion-content :fullscreen="true">
      <div class="home-bg">
        ...
        <!-- Loading -->
        <div v-if="cargando" class="barbers-scroll">
          <div v-for="i in 3" :key="i" class="barber-card skeleton"></div>
        </div>

        <!-- Error -->
        <div v-else-if="error" class="barbers-scroll">
          <div class="error-banner">
            <span>⚠️ Sin conexión al servidor</span>
          </div>
          <!-- Fallback con datos locales -->
          <div
            v-for="barber in barberosFallback"
            :key="barber.id"
            class="barber-card"
          ...
          >
            ...
            <button class="reserve-btn" @click.stop="reservar(barber)">Reservar</button>
          </div>
        </div>

        <!-- Datos reales del backend -->
        <div v-else class="barbers-scroll">
          <div
            v-for="barber in barberos"
            :key="barber.id"
            class="barber-card"
            @click="router.push(`/barbero/${barber.id}`)"
          >
            <div class="barber-avatar-card">
              <img
                :src="barber.fotoUrl || getFotoLocal(barber.id)"
                style="width:100%; height:100%; object-fit:cover; border-radius:10px"
                alt=""
                @error="(e: any) => e.target.src = getFotoLocal(barber.id)"
              />
            </div>
            <div class="barber-info">
              <div class="barber-name">{{ getNombre(barber) }}</div>
              <div class="barber-stars">
                <span v-for="i in 5" :key="i" :class="['star', i <= Math.round(barber.valoracion || 0) ? 'filled' : '']">★</span>
                <span class="rating-text">({{ (barber.valoracion || 0).toFixed(1) }})</span>
              </div>
            </div>
            <button class="reserve-btn" @click.stop="reservarReal(barber)">Reservar</button>
          </div>
        </div>
...
const fotosLocales = ['/assets/Rectangle.png', '/assets/Rectangle__5.png', '/assets/Rectangle__4.png']
const getFotoLocal = (id: number) => fotosLocales[(id - 1) % fotosLocales.length]

const getNombre = (barber: any) => {
  if (barber.persona?.nombre) return barber.persona.nombre
  return `Barbero #${barber.id}`
}

const barberosFallback = ref([
...
onMounted(async () => {
  try {
    ...
    console.warn('Backend no disponible, usando datos locales:', e)
    error.value = true
  } finally {
...
const reservarReal = (barber: any) => {
  bookingStore.barberoId = barber.id
  bookingStore.barberoNombre = getNombre(barber)
...
.home-bg { min-height: 100%; background: linear-gradient(160deg, #1a3a5c 0%, #0d1b2a 40%, #080f18 100%); padding-bottom: 30px; }
...
.barber-card { min-width: calc(100vw - 72px); margin-left: 0; flex-shrink: 0; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 14px 16px; display: flex; align-items: center; gap: 14px; cursor: pointer; scroll-snap-align: start; }
.barber-card.skeleton { height: 96px; background: rgba(255,255,255,0.04); animation: pulse 1.5s infinite; }
...
.error-banner { background: rgba(255,200,0,0.1); border: 1px solid rgba(255,200,0,0.3); border-radius: 12px; padding: 10px 16px; color: rgba(255,255,255,0.7); font-size: 13px; white-space: nowrap; flex-shrink: 0; display: flex; align-items: center; }
.section-content { padding: 4px 24px 0; }
.cta-btn { width: 100%; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.12); border-radius: 14px; color: white; padding: 18px; font-family: 'DM Sans', sans-serif; font-size: 16px; font-weight: 600; cursor: pointer; margin: 8px 0 28px; text-align: center; }
